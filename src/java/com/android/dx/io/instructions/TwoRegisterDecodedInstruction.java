...
public final class TwoRegisterDecodedInstruction extends DecodedInstruction {
	...
	public TwoRegisterDecodedInstruction(InstructionCodec format, int opcode,
			int index, IndexType indexType, int target, int relativeTarget,
			long literal, int a, int b, boolean decodedRegList) {
		super(format, opcode, index, indexType, target, relativeTarget,
				literal, decodedRegList);
	...
	public DecodedInstruction withIndex(int newIndex) {
		return new TwoRegisterDecodedInstruction(getFormat(), getOpcode(),
				newIndex, getIndexType(), getTarget(), getRelativeTarget(),
				getLiteral(), getA(), getB(), isDecodedFromRegisterList());
	}
