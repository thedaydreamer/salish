...
public final class FillArrayDataInsn extends Insn {
	 ...
	 * @param initValues
	 *            {@code non-null;} list of initial values to fill the array
	 * @param cst
	 *            {@code non-null;} type of the new array
	 */
	public FillArrayDataInsn(Rop opcode, SourcePosition position,
			RegisterSpecList sources, ArrayList<Constant> initValues,
			Constant cst) {
	...
	public Insn withRegisterOffset(int delta) {
		return new FillArrayDataInsn(getOpcode(), getPosition(), getSources()
				.withOffset(delta), initValues, arrayType);
	}
	...
	public Insn withNewRegisters(RegisterSpec result, RegisterSpecList sources) {
		...
		return new FillArrayDataInsn(getOpcode(), getPosition(), sources,
