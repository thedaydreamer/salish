...
public final class Form12x extends InsnFormat {
	...
	public String insnArgString(DalvInsn insn) {
		...
		/*
		 * The (sz - 2) and (sz - 1) below makes this code work for both the
		 * two- and three-register ops. (See "case 3" in isCompatible(), below.)
		 */
	...
	public boolean isCompatible(DalvInsn insn) {
		...
		case 3: {
			/*
			 * This format is allowed for ops that are effectively 3-arg but
			 * where the first two args are identical.
			 */
			rs1 = regs.get(1);
			rs2 = regs.get(2);
	...
	public void writeTo(AnnotatedOutput out, DalvInsn insn) {
		...
		/*
		 * The (sz - 2) and (sz - 1) below makes this code work for both the
		 * two- and three-register ops. (See "case 3" in isCompatible(), above.)
		 */

		write(out,
				opcodeUnit(
						insn,
						makeByte(regs.get(sz - 2).getReg(), regs.get(sz - 1)
								.getReg())));
	}
}
