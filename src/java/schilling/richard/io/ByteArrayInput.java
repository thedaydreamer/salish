 ...
 * 
 * <p>
 * <b>Note:</b> As per the {@link Input } interface, multi-byte reads all use
 * little-endian order.
 * </p>
 */
public class ByteArrayInput implements Input {
	...
	public int readShort() {
		int readAt = cursor;
		int result = ((data[readAt++] & 0xff) + ((data[readAt++] & 0xff) << 8));
		cursor = readAt;
		return result;
	}
	...
	public int readUnsignedOrSignedLeb128() {
		...
		if (result > 0x7f) {
			...
			// TODO move this block up to a spot before the if statement for
			// clarity.
			cursor = end;
			return result;
		}
		...
		// If the last byte is 0, then this was an unsigned value (incorrectly)
		// written in a signed format
		// The caller wants to know if this is the case, so we'll return the
		// negated value instead
		// If there was only a single byte that had a value of 0, then we would
		// have returned in the above
		// "else"
		if (data[end - 1] == 0) {
			return ~result;
		}
	...
	/**
	 * Throws the excpetion for when an attempt is made to read past the end of
	 * the instance.
	 */
