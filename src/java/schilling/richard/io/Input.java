...
package schilling.richard.io;

/**
 * Interface for a source for binary input. This is similar to
 * <code>java.util.DataInput</code>, but no <code>IOExceptions</code> are
 * declared, and multibyte input is defined to be little-endian.
 */
public interface Input {
	...
	/**
	 * Reads a DWARFv3-style signed LEB128 integer. For details, see the
	...
	/**
	 * Reads a DWARFv3-style unsigned LEB128 integer. For details, see the
	 * "Dalvik Executable Format" document or DWARF v3 section 7.6.
	 * 
	 * @return the integer value that was read
	 */
	 ...
	 * 
	 * @return If the value was formatted as a ULEB128, it returns the actual
	 *         unsigned value. Otherwise, if the value was formatted as a signed
	 *         LEB128, it negates the "correct" unsigned value and returns that
	 */
	 ...
	 * 
	 * @param bytes
	 *            non-null; the buffer to read the data into
	 * @param offset
	 *            &gt;= 0; offset into <code>bytes</code> for the first byte to
	 *            write
	 * @param length
	...
	/**
	 * reads a <code>byte[]</code> from this instance. This is just a convenient
	 * shorthand for <code>read(bytes, 0, bytes.length)</code>.
	 * 
	 * @param bytes
	 *            non-null; the buffer to read the data into
	 */
	...
	/**
	 * reads and decodes a null terminated utf8 string from the current cursor
	 * up to but not including the next null (0) byte. The terminating null byte
	 * is read and discarded, so that after the read, the cursor is positioned
	 * at the byte immediately after the terminating null
	 * 
	...
	/**
	 * Skip extra bytes if necessary to force alignment of the output cursor as
	 * given.
	 * 
	 * @param alignment
	 *            &gt; 0; the alignment; must be a power of two
	 */
