...
public class DexBufferInput {
     ...
     * 
     * @param buffer the buffer to read class data from.
     ...
     * 
     * @param buffer the buffer to read classes from.
     * @throws VerifyException
     */
     ...
     * 
     * @param buffer the buffer to read strings from.
     */
     ...
     * 
     * @param section the section to read an annotations directory from.
     */
    private DexModel readAnnotationsDirectoryItem(Clazz clazz, int offset) {
         ...
         * convert_annotation_set_item(classAnnotationsOffset); for (int i = 0;
         * i < fieldSz; i++) { int fieldIdx = section.readInt(); FieldId fid =
         * fieldIds.get(fieldIdx); Field field = lookup(fid); int
         * annotationOffset = section.readInt(); List<Annotation>
         * fieldAnnotations = convert_annotation_set_item(annotationOffset);
         * field.setAnnotations(fieldAnnotations); } for (int i = 0; i <
         * methodSz; i++) { int methodIdx = section.readInt(); MethodId mid =
         * methodIds.get(methodIdx); Method method = lookup(mid); int
         * annotationOffset = section.readInt(); List<Annotation>
         * methodAnnotations = convert_annotation_set_item(annotationOffset);
         * method.setAnnotations(methodAnnotations); } for (int i = 0; i <
         * paramSz; i++) { int methodIdx = section.readInt(); MethodId mid =
         * methodIds.get(methodIdx); Method method = lookup(mid); int
         * annotationSetRefOffset = section.readInt(); List<List<Annotation>>
         * parameterAnnotations = convert_annotation_set_ref_list( buffer,
         * annotationSetRefOffset);
    ...
    private List<Annotation> convert_annotation_set_item(int offset) {
        ...
        for (com.android.dx.io.Annotation annotation : annotations) {
             ...
             * annotation.getValues(); for (int i = 0; i < names.length; i++) {
             * String name = strings.get(names[i]); EncodedValue value =
             * values[i]; convertedAnn.annotationValues.put(name, value); }
