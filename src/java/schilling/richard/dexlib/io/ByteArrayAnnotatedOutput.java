...
import schilling.richard.io.ByteArray;
...
public final class ByteArrayAnnotatedOutput implements AnnotatedOutput {
	...
	/**
	 * null-ok; list of annotations, or <code>null</code> if this instance isn't
	 * keeping them
	 */
	...
	/**
	 * &gt;= 8 (if used); the number of bytes of hex output to use in
	 * annotations
	 */
	private int hexCols;
	...
	/**
	 * Gets the underlying <code>byte[]</code> of this instance, which may be
	 * larger than the number of bytes written
	 * 
	 * @see #toByteArray
	 * 
	...
	/**
	 * Constructs and returns a new <code>byte[]</code> that contains the
	 * written contents exactly (that is, with no extra unwritten bytes at the
	 * end).
	 * 
	 * @see #getArray
	 * 
	...
	public void writeZeroes(int count) {
		...
		/*
		 * There is no need to actually write zeroes, since the array is already
		 * preinitialized with zeroes.
		 */
	 ...
	 *            &gt;= 40; the desired maximum annotation width
	 * @param verbose
	...
	public void writeAnnotationsTo(Writer out) throws IOException {
		...
		while ((leftAt < cursor) && (rightAt < rightSz)) {
			...
			if (leftAt < start) {
				...
				text = padding.substring(0, a.getIndent() * this.indentAmount)
						+ a.getText();
				rightAt++;
			}
		...
		if (leftAt < cursor) {
			...
			left.write(Hex.dump(data, leftAt, cursor - leftAt, leftAt, hexCols,
					6));
		}
	 ...
	 * 
	 * @param desiredSize
	 *            &gt;= 0; the desired minimum total size of the array
	 */
	...
	private static class Annotation {
		 ...
		 * @param end
		 *            &gt;= start; end of annotated range (exclusive) or
		 *            <code>Integer.MAX_VALUE</code> if unclosed
		 * @param text
		 *            non-null; annotation text
		 */
