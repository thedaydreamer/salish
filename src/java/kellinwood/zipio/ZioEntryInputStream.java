...
public class ZioEntryInputStream extends InputStream {
	...
	public ZioEntryInputStream(ZioEntry entry) throws IOException {
		...
		if (dpos >= 0) {
			if (debug)
				log.debug(String.format("Seeking to %d",
						entry.getDataPosition()));
	...
	private int readBytes(byte[] b, int off, int len) throws IOException {
		...
		if (debug)
			log.debug(String.format("Read %d bytes for read(b,%d,%d)", numRead,
					off, len));
		return numRead;
	}
	...
	public long skip(long n) throws IOException {
		...
		if (debug)
			log.debug(String.format("Skipped %d bytes", numToSkip));
		return numToSkip;
	}
}
