...
/**
 * Produces the classic hex dump with an address column, hex data section (16
 * bytes per row) and right-column printable character dislpay.
 */
public class HexDumpEncoder {
	...
	public static String encode(byte[] data) {
		...
		for (int i = 0; i < data.length; i += 32) {
			...
			int max = Math.min(i + 32, data.length);
			...
			for (int j = i; j < max; j += 2) {
				hexOut.append(Character.valueOf((char) data[j]));
				hexOut.append(Character.valueOf((char) data[j + 1]));
				if ((j + 2) % 4 == 0)
					hexOut.append(' ');
