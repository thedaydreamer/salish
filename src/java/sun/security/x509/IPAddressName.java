 ...
 * object. Both IPv4 and IPv6 addresses are supported using the formats
 * specified in IETF PKIX RFC2459.
 * <p>
 * [RFC2459 4.2.1.7 Subject Alternative Name] When the subjectAltName extension
 * contains a iPAddress, the address MUST be stored in the octet string in
 * "network byte order," as specified in RFC 791. The least significant bit
 * (LSB) of each octet is the LSB of the corresponding byte in the network
 * address. For IP Version 4, as specified in RFC 791, the octet string MUST
 * contain exactly four octets. For IP Version 6, as specified in RFC 1883, the
 * octet string MUST contain exactly sixteen octets.
 * <p>
 * [RFC2459 4.2.1.11 Name Constraints] The syntax of iPAddress MUST be as
 * described in section 4.2.1.7 with the following additions specifically for
 * Name Constraints. For IPv4 addresses, the ipAddress field of generalName MUST
 * contain eight (8) octets, encoded in the style of RFC 1519 (CIDR) to
 * represent an address range.[RFC 1519] For IPv6 addresses, the ipAddress field
 * MUST contain 32 octets similarly encoded. For example, a name constraint for
 * "class C" subnet 10.9.8.0 shall be represented as the octets 0A 09 08 00 FF
 * FF FF 00, representing the CIDR notation 10.9.8.0/255.255.255.0.
 * <p>
 * 
 * @see GeneralName
...
public class IPAddressName implements GeneralNameInterface {
	...
	public IPAddressName(byte[] address) throws IOException {
		/*
		 * A valid address must consist of 4 bytes of address and optional 4
		 * bytes of 4 bytes of mask, or 16 bytes of address and optional 16
		 * bytes of mask.
		 */
	...
	/**
	 * Create an IPAddressName from a String. [IETF RFC1338 Supernetting & IETF
	 * RFC1519 Classless Inter-Domain Routing (CIDR)] For IPv4 addresses, the
	 * forms are "b1.b2.b3.b4" or "b1.b2.b3.b4/m1.m2.m3.m4", where b1 - b4 are
	 * decimal byte values 0-255 and m1 - m4 are decimal mask values 0 - 255.
	 * <p>
	 * [IETF RFC2373 IP Version 6 Addressing Architecture] For IPv6 addresses,
	 * the forms are "a1:a2:...:a8" or "a1:a2:...:a8/n", where a1-a8 are
	 * hexadecimal values representing the eight 16-bit pieces of the address.
	 * If /n is used, n is a decimal number indicating how many of the leftmost
	 * contiguous bits of the address comprise the prefix for this subnet.
	 * Internally, a mask value is created using the prefix length.
	 * <p>
	 * 
	 * @param name
	 *            String form of IPAddressName
	 * @throws IOException
	 *             if name can not be converted to a valid IPv4 or IPv6 address
	 */
	 ...
	 * 
	 * @param name
	 *            IPv4 address with optional mask values
	 * @throws IOException
	 *             on error
	 */
	private void parseIPv4(String name) throws IOException {
		...
		if (slashNdx == -1) {
			...
			// parse mask
			byte[] mask = InetAddress.getByName(name.substring(slashNdx + 1))
					.getAddress();

			// parse base address
			byte[] host = InetAddress.getByName(name.substring(0, slashNdx))
					.getAddress();
	 ...
	 * 
	 * @param name
	 *            String IPv6 address with optional /<prefix length> If /<prefix
	 *            length> is present, address[] array will be 32 bytes long,
	 *            otherwise 16.
	 * @throws IOException
	 *             on error
	 */
	...
	private void parseIPv6(String name) throws IOException {
		...
		if (slashNdx == -1) {
			...
			address = new byte[32];
			byte[] base = InetAddress.getByName(name.substring(0, slashNdx))
					.getAddress();
	...
	/**
	 * Return a standard String representation of IPAddress. See
	 * IPAddressName(String) for the formats used for IPv4 and IPv6 addresses.
	 * 
	 * @throws IOException
	 *             if the IPAddress cannot be converted to a String
	 */
	 ...
	 * <ul>
	 * <li>NAME_DIFF_TYPE = -1: input name is different type from name (i.e.
	 * does not constrain).
	 ...
	 * subtree)
	 * <li>NAME_SAME_TYPE = 3: input name does not match or narrow name, but is
	 * same type.
	 * </ul>
	 * . These results are used in checking NameConstraints during certification
	 * path verification.
	 * <p>
	 * [RFC2459] The syntax of iPAddress MUST be as described in section 4.2.1.7
	 * with the following additions specifically for Name Constraints. For IPv4
	 * addresses, the ipAddress field of generalName MUST contain eight (8)
	 * octets, encoded in the style of RFC 1519 (CIDR) to represent an address
	 * range.[RFC 1519] For IPv6 addresses, the ipAddress field MUST contain 32
	 * octets similarly encoded. For example, a name constraint for "class C"
	 * subnet 10.9.8.0 shall be represented as the octets 0A 09 08 00 FF FF FF
	 * 00, representing the CIDR notation 10.9.8.0/255.255.255.0.
	 * <p>
	 * 
	 * @param inputName
			...
			if (otherAddress.length == 4 && address.length == 4)
				...
				for (int i = 0; i < maskOffset; i++) {
					...
					if ((byte) (otherAddress[i] & otherAddress[i + maskOffset]) != otherAddress[i])
						otherEmpty = true;
					if (!(((byte) (address[i + maskOffset] & otherAddress[i
							+ maskOffset]) == address[i + maskOffset]) && ((byte) (address[i] & address[i
							+ maskOffset]) == (byte) (otherAddress[i] & address[i
							+ maskOffset])))) {
						otherSubsetOfThis = false;
					}
					if (!(((byte) (otherAddress[i + maskOffset] & address[i
							+ maskOffset]) == otherAddress[i + maskOffset]) && ((byte) (otherAddress[i] & otherAddress[i
							+ maskOffset]) == (byte) (address[i] & otherAddress[i
							+ maskOffset])))) {
				...
				for (; i < maskOffset; i++) {
					// Mask this address by other address mask and compare to
					// other address
					// If all match, then this address is in other address
					// subnet
				...
				int maskOffset = address.length / 2;
				for (; i < maskOffset; i++) {
					// Mask other address by this address mask and compare to
					// this address
