...
/**
 * This class defines the Extended Key Usage Extension, which indicates one or
 * more purposes for which the certified public key may be used, in addition to
 ...
 * <p>
 * 
 * If the extension is flagged critical, then the certificate MUST be used only
 ...
 * advisory field and does not imply that usage of the key is restricted by the
 * certification authority to the purpose indicated. Certificate using
 * applications may nevertheless require that a particular purpose be indicated
 ...
 * extended key usage field, then both fields MUST be processed independently
 ...
 * @since 1.4
 */
public class ExtendedKeyUsageExtension extends Extension implements
		CertAttrSet<String> {
	...
	private static final int[] emailProtectionOidData = { 1, 3, 6, 1, 5, 5, 7,
			3, 4 };
	private static final int[] ipsecEndSystemOidData = { 1, 3, 6, 1, 5, 5, 7,
			3, 5 };
	private static final int[] ipsecTunnelOidData = { 1, 3, 6, 1, 5, 5, 7, 3, 6 };
	private static final int[] ipsecUserOidData = { 1, 3, 6, 1, 5, 5, 7, 3, 7 };
	private static final int[] timeStampingOidData = { 1, 3, 6, 1, 5, 5, 7, 3,
			8 };
	private static final int[] OCSPSigningOidData = { 1, 3, 6, 1, 5, 5, 7, 3, 9 };

	static {
		map.put(ObjectIdentifier.newInternal(anyExtendedKeyUsageOidData),
				"anyExtendedKeyUsage");
		...
		map.put(ObjectIdentifier.newInternal(emailProtectionOidData),
				"emailProtection");
		map.put(ObjectIdentifier.newInternal(ipsecEndSystemOidData),
				"ipsecEndSystem");
		...
		map.put(ObjectIdentifier.newInternal(timeStampingOidData),
				"timeStamping");
	...
	/**
	 * Create a ExtendedKeyUsageExtension object from a Vector of Key Usages;
	 * the criticality is set to false.
	 * 
	 * @param keyUsages
	 *            the Vector of KeyUsages (ObjectIdentifiers)
	 */
	...
	/**
	 * Create a ExtendedKeyUsageExtension object from a Vector of KeyUsages with
	...
	public ExtendedKeyUsageExtension(Boolean critical,
			Vector<ObjectIdentifier> keyUsages) throws IOException {
		...
		if (name.equalsIgnoreCase(USAGES)) {
			if (!(obj instanceof Vector)) {
				throw new IOException(
						"Attribute value should be of type Vector.");
			}
