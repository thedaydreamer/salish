...
/**
 * Represents Netscape Certificate Type Extension. The details are defined <a
 * href=http://www.netscape.com/eng/security/comm4-cert-exts.html> here </a>.
 * 
 * <p>
 * This extension, if present, defines both the purpose (e.g., encipherment,
 * signature, certificate signing) and the application (e.g., SSL, S/Mime or
 * Object Signing of the key contained in the certificate. This extension has
 * been superseded by IETF PKIX extensions but is provided here for
...
public class NetscapeCertTypeExtension extends Extension implements
		CertAttrSet<String> {
	...
	private static MapEntry[] mMapData = { new MapEntry(SSL_CLIENT, 0),
			new MapEntry(SSL_SERVER, 1), new MapEntry(S_MIME, 2),
			new MapEntry(OBJECT_SIGNING, 3),
			// note that bit 4 is reserved
			new MapEntry(SSL_CA, 5), new MapEntry(S_MIME_CA, 6),
	...
	/**
	 * Create a NetscapeCertTypeExtension with the passed bit settings. The
	 * criticality is set to true.
	 * 
	 * @param bitString
	 *            the bits to be set for the extension.
	 */
	...
	/**
	 * Get a boolean array representing the bits of this extension, as it maps
	 * to the KeyUsage extension.
	 * 
	 * @return the bit values of this extension mapped to the bit values of the
	 *         KeyUsage extension as an array of booleans.
	 */
	public boolean[] getKeyUsageMappedBits() {
		...
		try {
			if (isSet(getPosition(SSL_CLIENT)) || isSet(getPosition(S_MIME))
			...
			if (isSet(getPosition(SSL_CA)) || isSet(getPosition(S_MIME_CA))
