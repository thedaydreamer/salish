...
public class X509CertImpl extends X509Certificate implements DerEncoder {
	...
	public static final String VERSION = NAME + DOT + INFO + DOT
			+ X509CertInfo.VERSION + DOT + CertificateVersion.VERSION;
	...
	/**
	 * PublicKey that has previously been used to verify the signature of this
	 * certificate. Null if the certificate has not yet been verified.
	 */
	private PublicKey verifiedPublicKey;
	/**
	 * If verifiedPublicKey is not null, name of the provider used to
	 * successfully verify the signature of this certificate, or the empty
	 * String if no provider was explicitly specified.
	 */
	...
	/**
	 * unmarshals an X.509 certificate from an input stream. If the certificate
	 * is RFC1421 hex-encoded, then it must begin with the line
	 * X509Factory.BEGIN_CERT and end with the line X509Factory.END_CERT.
	 * 
	 * @param in
	 *            an input stream holding at least one certificate that may be
	 *            either DER-encoded or RFC1421 hex-encoded version of the
	...
	public X509CertImpl(InputStream in) throws CertificateException {
		...
		try {
			...
			try {
				...
				CertificateException ce = new CertificateException(
						"Input stream must be " + "either DER-encoded bytes "
		...
		try {
			...
			signedCert = null;
			CertificateException ce = new CertificateException(
					"Unable to parse DER value of " + "certificate, " + ioe);
			ce.initCause(ioe);
			throw ce;
		}
	}
	 ...
	 * @throws IOException
	 *             if stream can not be interpreted as RFC1421 encoded bytes
	 */
	private DerValue readRFC1421Cert(InputStream in) throws IOException {
		DerValue der = null;
		String line = null;
		BufferedReader certBufferedReader = new BufferedReader(
	 ...
	 * 
	 * @param derVal
	 *            the der value containing the encoded cert.
	 * @exception CertificateException
	 *                on parsing and initialization errors.
	 */
	 ...
	 * 
	 * @param out
	 *            an input stream to which the certificate is appended.
	 ...
	 * that they neither modify it nor expose it to untrusted code.
	 */
	 ...
	 * 
	 * Note that the X509CertInfo is not cloned for performance reasons. Callers
	 * must ensure that they do not modify it. All other attributes are cloned.
	 * 
	 * @param name
	 ...
	 * X509Certificate with a slightly more efficient version that is also aware
	 * of X509CertImpl mutability.
	 */
	...
	public X500Principal getIssuerX500Principal() {
		...
		try {
			X500Principal issuer = (X500Principal) info
					.get(CertificateIssuerName.NAME + DOT
	 ...
	 * 
	 * @return AuthorityKeyIdentifier object or null (if no such object in
	 *         certificate)
	 */
	 ...
	 * 
	 * @return ExtendedKeyUsage extension object or null (if no such object in
	 *         certificate)
	 */
	 ...
	 * 
	 * @return PolicyMappingsExtension object or null (if no such object in
	 *         certificate)
	 */
	...
	/**
	 * Return true if a critical extension is found that is not supported,
	...
	/**
	 * Gets a Set of the extension(s) marked CRITICAL in the certificate. In the
	 * returned set, each extension is represented by its OID string.
	 * 
	...
	/**
	 * Gets a Set of the extension(s) marked NON-CRITICAL in the certificate. In
	 * the returned set, each extension is represented by its OID string.
	 * 
	 * @return a set of the extension oid strings in the certificate that are
	 *         NOT marked critical.
	 */
	 ...
	 * @return Extension or null if certificate does not contain this extension
	 */
	public Extension getExtension(ObjectIdentifier oid) {
		...
		try {
			...
			try {
				extensions = (CertificateExtensions) info
						.get(CertificateExtensions.NAME);
	...
	public Extension getUnparseableExtension(ObjectIdentifier oid) {
		...
		try {
			...
			if (extensions == null) {
				...
				return extensions.getUnparseableExtensions()
						.get(oid.toString());
			}
		} catch (IOException ioe) {
			return null;
		...
		try {
			...
			if (extAlias == null) { // may be unknown
				...
				try {
					...
					// get() throws an Exception instead of returning null,
					// ignore
				}
			}
			if (certExt == null) {
	...
	/**
	 * This method are the overridden implementation of getExtendedKeyUsage
	 * method in X509Certificate in the Sun provider. It is better
	 * performance-wise since it returns cached values.
	 */
	public synchronized List<String> getExtendedKeyUsage()
		...
		if (readOnly && extKeyUsage != null) {
			...
			extKeyUsage = Collections.unmodifiableList(ext
					.getExtendedKeyUsage());
			return extKeyUsage;
		}
	}
	 ...
	 * getExtendedKeyUsage method in X509Certificate. A X509Certificate provider
	 * generally should overwrite this to provide among other things caching for
	 * better performance.
	 */
	public static List<String> getExtendedKeyUsage(X509Certificate cert)
		...
		try {
			...
			ExtendedKeyUsageExtension ekuExt = new ExtendedKeyUsageExtension(
					Boolean.FALSE, data);
	...
	public int getBasicConstraints() {
		try {
			String extAlias = OIDMap
					.getName(PKIXExtensions.BasicConstraints_Id);
			if (extAlias == null)
				return -1;
	 ...
	 * {@link #getSubjectAlternativeNames} or {@link #getIssuerAlternativeNames}
	 * .
	 * 
	 * @param names
	 *            the GeneralNames to be converted
	...
	/**
	 * Checks a Collection of altNames and clones any name entries of type byte
	 * [].
	 */
	// only partially generified due to javac bug
	private static Collection<List<?>> cloneAltNames(
		...
		if (mustClone) {
			...
			for (List<?> nameEntry : altNames) {
				...
				if (nameObject instanceof byte[]) {
					List<Object> nameEntryCopy = new ArrayList<Object>(
							nameEntry);
	 ...
	 * getSubjectAlternativeNames method in X509Certificate in the Sun provider.
	 * It is better performance-wise since it returns cached values.
	 */
	...
	/**
	 * This static method is the default implementation of the
	 * getSubjectAlternaitveNames method in X509Certificate. A X509Certificate
	 * provider generally should overwrite this to provide among other things
	...
	/**
	 * This method are the overridden implementation of
	 * getIssuerAlternativeNames method in X509Certificate in the Sun provider.
	 * It is better performance-wise since it returns cached values.
	 */
	public synchronized Collection<List<?>> getIssuerAlternativeNames()
		...
		GeneralNames names;
		try {
			names = (GeneralNames) issuerAltNameExt
	...
	/**
	 * This static method is the default implementation of the
	 * getIssuerAlternaitveNames method in X509Certificate. A X509Certificate
	 * provider generally should overwrite this to provide among other things
	 * caching for better performance.
	 */
	public static Collection<List<?>> getIssuerAlternativeNames(
			X509Certificate cert) throws CertificateParsingException {
		try {
	...
	private void parse(DerValue val) throws CertificateException, IOException {
		...
		AlgorithmId infoSigAlg = (AlgorithmId) info
				.get(CertificateAlgorithmId.NAME + DOT
	 ...
	 * Parses the encoded form of the cert to preserve the principal's ASN.1
	 * encoding.
	 */
	...
	/**
	 * Extract the subject X500Principal from an X509Certificate. Called from
	...
	/**
	 * Extract the issuer X500Principal from an X509Certificate. Called from
	...
	/**
	 * Returned the encoding of the given certificate for internal use. Callers
	 * must guarantee that they neither modify it nor expose it to untrusted
	 * code. Uses getEncodedInternal() if the certificate is instance of
	 * X509CertImpl, getEncoded() otherwise.
	 */
	...
	/**
	 * Utility method to convert an arbitrary instance of X509Certificate to a
	 * X509CertImpl. Does a cast if possible, otherwise reparses the encoding.
	 */
	...
	/**
	 * Return an interned X509CertImpl for the given certificate. If the given
	 * X509Certificate or X509CertImpl is already present in the cert cache, the
	 * cached object is returned. Otherwise, if it is a X509Certificate, it is
	 * first converted to a X509CertImpl. Then the X509CertImpl is added to the
	 * cache and returned.
	 * 
	 * Note that all certificates created via generateCertificate(InputStream)
	 * are already interned and this method does not need to be called. It is
	 * useful for certificates that cannot be created via generateCertificate()
	 * and for converting other X509Certificate implementations to an
	 * X509CertImpl.
	 */
	...
	/**
	 * Utility method to test if a certificate is self-issued. This is the case
	 * iff the subject and issuer X500Principals are equal.
	 */
	...
	/**
	 * Utility method to test if a certificate is self-signed. This is the case
	 * iff the subject and issuer X500Principals are equal AND the certificate's
	 * subject public key can be used to verify the certificate. In case of
