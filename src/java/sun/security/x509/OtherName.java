...
/**
 * This class represents the OtherName as required by the GeneralNames ASN.1
 * object. It supplies the generic framework to allow specific Other Name types,
 * and also provides minimal support for unrecognized Other Name types.
 * 
...
public class OtherName implements GeneralNameInterface {
	...
	/**
	 * Create the OtherName object from a passed ObjectIdentfier and byte array
	 * name value
	 * 
	 * @param oid
	 *            ObjectIdentifier of this OtherName object
	 * @param value
	 *            the DER-encoded value of the OtherName
	 * @throws IOException
	 *             on error
	 */
	...
	private GeneralNameInterface getGNI(ObjectIdentifier oid, byte[] nameValue)
		...
		try {
			...
			GeneralNameInterface gni = (GeneralNameInterface) cons
					.newInstance(passed);
			return gni;
		} catch (Exception e) {
			throw (IOException) new IOException("Instantiation error: " + e)
					.initCause(e);
		}
	}
	 ...
	 * 
	 * @param out
	 *            the DER stream to encode the Other-Name to.
	 * @exception IOException
	 *                on encoding errors.
	 */
	public void encode(DerOutputStream out) throws IOException {
		if (gni != null) {
			...
			tmp.putOID(oid);
			tmp.write(
					DerValue.createTag(DerValue.TAG_CONTEXT, true, TAG_VALUE),
					nameValue);
	 ...
	 *             if name is same type, but comparison operations are not
	 *             supported for this name type.
	 */
