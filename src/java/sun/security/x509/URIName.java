 ...
 * stored in the uniformResourceIdentifier (an IA5String). The name MUST be a
 * non-relative URL, and MUST follow the URL syntax and encoding rules specified
 * in [RFC 1738]. The name must include both a scheme (e.g., "http" or "ftp")
 * and a scheme-specific-part. The scheme- specific-part must include a fully
 * qualified domain name or IP address as the host.
 * <p>
 * As specified in [RFC 1738], the scheme name is not case-sensitive (e.g.,
 * "http" is equivalent to "HTTP"). The host part is also not case-sensitive,
 * but other components of the scheme-specific-part may be case-sensitive. When
 * comparing URIs, conforming implementations MUST compare the scheme and host
 * without regard to case, but assume the remainder of the scheme-specific-part
 * is case sensitive.
 * <p>
 ...
 * </pre>
 * 
 * A URL contains the name of the scheme being used (<scheme>) followed by a
 * colon and then a string (the <scheme-specific-part>) whose interpretation
 ...
 * </pre>
 * 
 * [RFC2732] specifies that an IPv6 address contained inside a URL must be
 * enclosed in square brackets (to allow distinguishing the colons that separate
 * IPv6 components from the colons that separate scheme-specific data.
 * <p>
 * 
 * @author Amit Kapoor
...
public class URIName implements GeneralNameInterface {
	 ...
	 *            the URIName.
	 * @throws IOException
	 *             if name is not a proper URIName
	 */
	public URIName(String name) throws IOException {
		try {
			uri = new URI(name);
		} catch (URISyntaxException use) {
			throw (IOException) new IOException("invalid URI name:" + name)
					.initCause(use);
		}
	...
	/**
	 * Create the URIName object with the specified name constraint. URI name
	 * constraints syntax is different than SubjectAltNames, etc. See 4.2.1.11
	 * of RFC 3280.
	 * 
	 * @param value
	 *            the URI name constraint
	 * @throws IOException
	 *             if name is not a proper URI name constraint
	 */
	public static URIName nameConstraint(DerValue value) throws IOException {
		...
		try {
			...
			throw (IOException) new IOException("invalid URI name constraint:"
					+ name).initCause(use);
		}
		if (uri.getScheme() == null) {
	 ...
	 * 
	 * @param out
	 *            the DER stream to encode the URIName to.
	 * @exception IOException
	 *                on encoding errors.
	 */
	...
	/**
	 * Return the host object type; if host name is a DNSName, then this host
	 * object does not include any initial "." on the name.
	 * 
	 ...
	 * "foo.bar.com"; and ".xyz.com". When the the constraint begins with a
	 * period, it may be expanded with one or more subdomains. That is, the
	 * constraint ".xyz.com" is satisfied by both abc.xyz.com and
	 * abc.def.xyz.com. However, the constraint ".xyz.com" is not satisfied by
	 * "xyz.com". When the constraint does not begin with a period, it specifies
	 * a host.
	 * <p>
	 * 
	 * @param inputName
