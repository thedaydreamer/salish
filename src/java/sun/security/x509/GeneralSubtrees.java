...
public class GeneralSubtrees implements Cloneable {
	 ...
	 * 
	 * @param other
	 *            GeneralSubtrees to compare to this
	...
	/**
	 * Return the GeneralNameInterface form of the GeneralName in one of the
	 * GeneralSubtrees.
	 * 
	 * @param ndx
	 *            index of the GeneralSubtree from which to obtain the name
	 */
	private GeneralNameInterface getGeneralNameInterface(int ndx) {
		return getGeneralNameInterface(get(ndx));
	}

	private static GeneralNameInterface getGeneralNameInterface(
			GeneralSubtree gs) {
	...
	/**
	 * minimize this GeneralSubtrees by removing all redundant entries. Internal
	 * method used by intersect and reduce.
	 */
	...
	/**
	 * create a subtree containing an instance of the input name type that
	 * widens all other names of that type.
	 * 
	...
	private GeneralSubtree createWidestSubtree(GeneralNameInterface name) {
		try {
			...
			case GeneralNameInterface.NAME_OID:
				newName = new GeneralName(new OIDName(new ObjectIdentifier(
						(int[]) null)));
				break;
			default:
				throw new IOException("Unsupported GeneralNameInterface type: "
						+ name.getType());
			}
	...
	/**
	 * intersect this GeneralSubtrees with other. This function is used in
	 * merging permitted NameConstraints. The operation is performed as follows:
	 * <ul>
	 * <li>If a name in other narrows all names of the same type in this, the
	 * result will contain the narrower name and none of the names it narrows.
	 * <li>If a name in other widens all names of the same type in this, the
	 * result will not contain the wider name.
	 * <li>If a name in other does not share the same subtree with any name of
	 * the same type in this, then the name is added to the list of
	 * GeneralSubtrees returned. These names should be added to the list of
	 * names that are specifically excluded. The reason is that, if the
	 * intersection is empty, then no names of that type are permitted, and the
	 * only way to express this in NameConstraints is to include the name in
	 * excludedNames.
	 * <li>If a name in this has no name of the same type in other, then the
	 * result contains the name in this. No name of a given type means the name
	 * type is completely permitted.
	 * <li>If a name in other has no name of the same type in this, then the
	 * result contains the name in other. This means that the name is now
	 * constrained in some way, whereas before it was completely permitted.
	 * <ul>
	 * 
	 * @param other
	 *            GeneralSubtrees to be intersected with this
	 * @returns GeneralSubtrees to be merged with excluded; these are
	 *          empty-valued name types corresponding to entries that were of
	 *          the same type but did not share the same subtree between this
	 *          and other. Returns null if no such.
	 */
	public GeneralSubtrees intersect(GeneralSubtrees other) {

		if (other == null) {
			throw new NullPointerException(
					"other GeneralSubtrees must not be null");
		}
				...
				for (int j = 0; j < size(); j++) {
					...
					if (thisAltEntry.getType() == thisEntry.getType()) {
						for (int k = 0; k < other.size(); k++) {
							GeneralNameInterface othAltEntry = other
									.getGeneralNameInterface(k);

							int constraintType = thisAltEntry
									.constrains(othAltEntry);
	 ...
	 * 
	 * @param other
	 *            GeneralSubtrees to be united with this
	 */
	...
	/**
	 * reduce this GeneralSubtrees by contents of another. This function is used
	 * in merging excluded NameConstraints with permitted NameConstraints to
	 * obtain a minimal form of permitted NameConstraints. It is an
	...
	public void reduce(GeneralSubtrees excluded) {
		...
		for (int i = 0, n = excluded.size(); i < n; i++) {
			GeneralNameInterface excludedName = excluded
					.getGeneralNameInterface(i);
			for (int j = 0; j < size(); j++) {
