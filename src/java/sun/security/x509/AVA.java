 ...
 * generating) RFC 1779 or RFC 2253 syntax strings.
 * 
 * <P>
...
public class AVA implements DerEncoder {
	...
	/**
	 * DEFAULT format allows both RFC1779 and RFC2253 syntax and additional
	 * keywords.
	 */
	 ...
	 * Backslash and quote characters must also be individually escaped. Leading
	 * and trailing spaces, also multiple internal spaces, also call for quoting
	 * the whole string.
	 */
	private static final String specialChars = ",+=\n<>#;";

	/*
	 * In RFC2253, if the value has any of these characters in it, it must be
	 * quoted by a preceding \.
	 */
	...
	/**
	 * Parse an RFC 1779 or RFC 2253 style AVA string: CN=fee fie foe fum or
	 * perhaps with quotes. Not all defined AVA tags are supported; of current
	 * note are X.400 related ones (PRMD, ADMD, etc).
	 * 
	...
	/**
	 * Parse an RFC 1779 or RFC 2253 style AVA string: CN=fee fie foe fum or
	 * perhaps with quotes. Additional keywords can be specified in the
	 * keyword/OID map.
	 * 
	 * This terminates at unescaped AVA separators ("+") or RDN separators (",",
	 * ";"), or DN terminators (">"), and removes cosmetic whitespace at the end
	 ...
	 * 
	 * XXX format RFC1779 should only allow RFC1779 syntax but is actually
	 * DEFAULT with RFC1779 keywords.
	 */
	 ...
	 *            parsing format
	 * @param keywordMap
	 *            a Map where a keyword String maps to a corresponding OID
	 *            String. Each AVA keyword will be mapped to the corresponding
	 *            OID. If an entry does not exist, it will fallback to the
	 *            builtin keyword/OID mapping.
	 * @throws IOException
	 *             if the AVA String is not valid in the specified standard or
	 *             an OID String from the keywordMap is improperly formatted
	 */
	AVA(Reader in, int format, Map<String, String> keywordMap)
		 ...
		 * stripped away unless format is RFC2253.
		 */
		temp.setLength(0);
		if (format == RFC2253) {
			// read next character
	...
	private static DerValue parseHexString(Reader in, int format)
			throws IOException {
		...
		while (true) {
			...
			if (cVal == -1) {
				throw new IOException("AVA parse, invalid hex " + "digit: "
						+ (char) c);
			}
	...
	private DerValue parseQuotedString(Reader in, StringBuilder temp)
			throws IOException {
	...
	private DerValue parseString(Reader in, int c, int format,
			StringBuilder temp) throws IOException {
			...
			if (c == '\\') {
				...
				if (format == RFC2253) {
					if (specialChars2253.indexOf((char) c) != -1) {
						throw new IOException("Character '" + (char) c
	...
	/**
	 * Returns a printable form of this attribute, using RFC 1779 syntax for
	 * individual attribute/value assertions.
	 */
	...
	/**
	 * Returns a printable form of this attribute, using RFC 1779 syntax for
	 * individual attribute/value assertions. It only emits standardised
	 * keywords.
	 */
	...
	/**
	 * Returns a printable form of this attribute, using RFC 1779 syntax for
	 * individual attribute/value assertions. It emits standardised keywords, as
	 * well as keywords contained in the OID/keyword map.
	 */
	...
	/**
	 * Returns a printable form of this attribute, using RFC 2253 syntax for
	 * individual attribute/value assertions. It only emits standardised
	 * keywords.
	 */
	...
	public String toRFC2253String(Map<String, String> oidMap) {
		...
		/*
		 * Section 2.4: Converting an AttributeValue from ASN.1 to a String. If
		 * the AttributeValue is of a type which does not have a string
		...
		if ((typeAndValue.charAt(0) >= '0' && typeAndValue.charAt(0) <= '9')
			 ...
			 * string o a space character occurring at the end of the string o
	...
	public String toRFC2253CanonicalString() {
		...
		if ((typeAndValue.charAt(0) >= '0' && typeAndValue.charAt(0) <= '9')
			 ...
			 * 
			 * o a space or "#" character occurring at the beginning of the
			 * string o a space character occurring at the end of the string
			 * 
	...
	private String toKeywordValueString(String keyword) {
		 ...
		 * practical. First the keyword (mandatory), then the equals sign,
		 * finally the value.
		 */
			...
			if (valStr == null) {
				...
				for (int i = 0; i < valStr.length(); i++) {
					...
					if (DerValue.isPrintableStringChar(c)
						...
						byte[] valueBytes = Character.toString(c).getBytes(
								"UTF8");
						for (int j = 0; j < valueBytes.length; j++) {
							sbuffer.append('\\');
							char hexChar = Character.forDigit(
									0xF & (valueBytes[j] >>> 4), 16);
							sbuffer.append(Character.toUpperCase(hexChar));
							hexChar = Character.forDigit(0xF & (valueBytes[j]),
									16);
							sbuffer.append(Character.toUpperCase(hexChar));
						}
					} else {
...
/**
 * Helper class that allows conversion from String to ObjectIdentifier and vice
 * versa according to RFC1779, RFC2253, and an augmented version of those
 * standards.
 */
class AVAKeyword {
	...
	/**
	 * Get an object identifier representing the specified keyword (or string
	 * encoded object identifier) in the given standard.
	 * 
	 * @throws IOException
	 *             If the keyword is not valid in the specified standard
	 */
	static ObjectIdentifier getOID(String keyword, int standard)
			throws IOException {
		return getOID(keyword, standard,
				Collections.<String, String> emptyMap());
	}
	 ...
	 *            builtin keyword/OID mapping.
	 * @throws IOException
	 *             If the keyword is not valid in the specified standard or the
	 *             OID String to which a keyword maps to is improperly
	 *             formatted.
	 */
	static ObjectIdentifier getOID(String keyword, int standard,
			Map<String, String> extraKeywordMap) throws IOException {
	...
	/**
	 * Get a keyword for the given ObjectIdentifier according to standard. If no
	 * keyword is available, the ObjectIdentifier is encoded as a String.
	 */
	static String getKeyword(ObjectIdentifier oid, int standard) {
		return getKeyword(oid, standard,
				Collections.<String, String> emptyMap());
	}
	 ...
	 * builtin/default set. If no keyword is available, the ObjectIdentifier is
	 * encoded as a String.
	 */
	static String getKeyword(ObjectIdentifier oid, int standard,
			Map<String, String> extraOidMap) {
	...
	static {
		...
		new AVAKeyword("GENERATION", X500Name.GENERATIONQUALIFIER_OID, false,
				false);
		new AVAKeyword("EMAIL", PKCS9Attribute.EMAIL_ADDRESS_OID, false, false);
		new AVAKeyword("EMAILADDRESS", PKCS9Attribute.EMAIL_ADDRESS_OID, false,
				false);
