 ...
 * 
 * <p>
 * The extension is defined in Section 5.2.5 of <a
 * href="http://www.ietf.org/rfc/rfc3280.txt">Internet X.509 PKI Certific ate
 * and Certificate Revocation List (CRL) Profile</a>.
 * 
 * <p>
...
public class IssuingDistributionPointExtension extends Extension implements
	 ...
	 *            the name of the distribution point, or null for none.
	 * @param revocationReasons
	 *            the revocation reasons associated with the distribution point,
	 *            or null for none.
	 * @param hasOnlyUserCerts
	 *            if <code>true</code> then scope of the CRL includes only user
	 *            certificates.
	 * @param hasOnlyCACerts
	 *            if <code>true</code> then scope of the CRL includes only CA
	 *            certificates.
	 * @param hasOnlyAttributeCerts
	 *            if <code>true</code> then scope of the CRL includes only
	 *            attribute certificates.
	 * @param isIndirectCRL
	 *            if <code>true</code> then the scope of the CRL includes
	 *            certificates issued by authorities other than the CRL issuer.
	 *            The responsible authority is indicated by a certificate issuer
	 *            CRL entry extension.
	 * @throws IllegalArgumentException
	 *             if more than one of <code>hasOnlyUserCerts</code>,
	 *             <code>hasOnlyCACerts</code>,
	 *             <code>hasOnlyAttributeCerts</code> is set to
	 *             <code>true</code>.
	 * @throws IOException
	 *             on encoding error.
	 */
	public IssuingDistributionPointExtension(
			...
			ReasonFlags revocationReasons, boolean hasOnlyUserCerts,
			boolean hasOnlyCACerts, boolean hasOnlyAttributeCerts,
			boolean isIndirectCRL) throws IOException {
	 ...
	 * @param value
	 *            the DER-encoded value. It must be a <code>byte[]</code>.
		...
		while (in != null && in.available() != 0) {
			...
			if (opt.isContextSpecific(TAG_DISTRIBUTION_POINT)
				...
				hasOnlyCACerts = opt.getBoolean();
					...
					&& !opt.isConstructed()) {
				revocationReasons = new ReasonFlags(opt); // expects tag
															// implicit
	...
	private void encodeThis() throws IOException {
		...
		if (distributionPoint == null && revocationReasons == null
		...
		if (revocationReasons != null) {
			...
			tagged.writeImplicit(DerValue.createTag(DerValue.TAG_CONTEXT,
					false, TAG_ONLY_SOME_REASONS), tmp);
		}
		...
		if (hasOnlyAttributeCerts) {
			...
			tagged.writeImplicit(DerValue.createTag(DerValue.TAG_CONTEXT,
					false, TAG_ONLY_ATTRIBUTE_CERTS), tmp);
		}
	...
	public String toString() {
		...
		sb.append(
				(hasOnlyUserCerts) ? ("  Only contains user certs: true")
						: ("  Only contains user certs: false")).append("\n");

		sb.append(
				(hasOnlyCACerts) ? ("  Only contains CA certs: true")
						: ("  Only contains CA certs: false")).append("\n");

		sb.append(
				(hasOnlyAttributeCerts) ? ("  Only contains attribute certs: true")
						: ("  Only contains attribute certs: false")).append(
				"\n");

		sb.append(
				(isIndirectCRL) ? ("  Indirect CRL: true")
