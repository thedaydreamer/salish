 ...
 * certificate issuer defaults to the CRL issuer. On subsequent entries in an
 * indirect CRL, if this extension is not present, the certificate issuer for
 * the entry is the same as that for the preceding entry.
 * <p>
 * If used by conforming CRL issuers, this extension is always critical. If an
 * implementation ignored this extension it could not correctly attribute CRL
 * entries to certificates. PKIX (RFC 3280) RECOMMENDS that implementations
...
public class CertificateIssuerExtension extends Extension implements
	...
	/**
	 * Create a CertificateIssuerExtension from the specified DER encoded value
	 * of the same.
	 * 
	 * @param critical
