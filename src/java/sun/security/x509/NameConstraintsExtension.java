 ...
 * @see CertAttrSet
 */
public class NameConstraintsExtension extends Extension implements
		CertAttrSet<String>, Cloneable {
	/**
	...
	/**
	 * The default constructor for this class. Both parameters are optional and
	 * can be set to null. The extension criticality is set to true.
	 * 
	 * @param permitted
	 *            the permitted GeneralSubtrees (null for optional).
	 * @param excluded
	 *            the excluded GeneralSubtrees (null for optional).
	 */
	 ...
	 * 
	 * @param out
	 *            the OutputStream to write the extension to.
	 * @exception IOException
	 *                on encoding errors.
	 */
	...
	/**
	 * Merge additional name constraints with existing ones. This function is
	 * used in certification path processing to accumulate name constraints from
	 * successive certificates in the path. Note that NameConstraints can never
	 * be expanded by a merge, just remain constant or become more limiting.
	 * <p>
	 * IETF RFC2459 specifies the processing of Name Constraints as follows:
	 * <p>
	 * (j) If permittedSubtrees is present in the certificate, set the
	 * constrained subtrees state variable to the intersection of its previous
	 * value and the value indicated in the extension field.
	 * <p>
	 * (k) If excludedSubtrees is present in the certificate, set the excluded
	 * subtrees state variable to the union of its previous value and the value
	 * indicated in the extension field.
	 * <p>
	 * 
	 * @param newConstraints
	 *            additional NameConstraints to be applied
	 * @throws IOException
	 *             on error
	 */
	public void merge(NameConstraintsExtension newConstraints)
		...
		/*
		 * If excludedSubtrees is present in the certificate, set the excluded
		 * subtrees state variable to the union of its previous value and the
		 * value indicated in the extension field.
		 */

		GeneralSubtrees newExcluded = (GeneralSubtrees) newConstraints
				.get(EXCLUDED_SUBTREES);
		if (excluded == null) {
			excluded = (newExcluded != null) ? (GeneralSubtrees) newExcluded
					.clone() : null;
		} else {
			if (newExcluded != null) {
		...
		GeneralSubtrees newPermitted = (GeneralSubtrees) newConstraints
				.get(PERMITTED_SUBTREES);
		if (permitted == null) {
			permitted = (newPermitted != null) ? (GeneralSubtrees) newPermitted
					.clone() : null;
		} else {
	...
	/**
	 * check whether a certificate conforms to these NameConstraints. This
	 * involves verifying that the subject name and subjectAltName extension
	 * (critical or noncritical) is consistent with the permitted subtrees state
	 * variables. Also verify that the subject name and subjectAltName extension
	 * (critical or noncritical) is consistent with the excluded subtrees state
	 * variables.
	 * 
	 * @param cert
	 *            X509Certificate to be verified
	...
	public boolean verify(X509Certificate cert) throws IOException {
		...
		try {
			...
			SubjectAlternativeNameExtension altNameExt = certImpl
			...
			if (altNameExt != null) {
				...
				altNames = (GeneralNames) (altNameExt
						.get(altNameExt.SUBJECT_NAME));
			}
	...
	/**
	 * check whether a name conforms to these NameConstraints. This involves
	 * verifying that the name is consistent with the permitted and excluded
	 * subtrees variables.
	 * 
	 * @param name
	 *            GeneralNameInterface name to be verified
	 * @returns true if certificate verifies successfully
	 * @throws IOException
	 *             on error
	 */
	...
	/**
	 * Perform the RFC 822 special case check. We have a certificate that does
	 * not contain any subject alternative names. Check that any EMAILADDRESS
	 * attributes in its subject name conform to these NameConstraints.
	 * 
	 * @param subject
	...
	public Object clone() {
		try {
			NameConstraintsExtension newNCE = (NameConstraintsExtension) super
					.clone();
			...
			throw new RuntimeException(
					"CloneNotSupportedException while "
