 ...
 * href="http://www.ietf.org/rfc/rfc3280.txt">RFC 3280: Internet X.509 Public
...
public class X509CRLImpl extends X509CRL {
	...
	/**
	 * PublicKey that has previously been used to successfully verify the
	 * signature of this CRL. Null if the CRL has not yet been verified
	 * (successfully).
	 */
	private PublicKey verifiedPublicKey;
	/**
	 * If verifiedPublicKey is not null, name of the provider used to
	 * successfully verify the signature of this CRL, or the empty String if no
	 ...
	 * 
	 * @param val
	 *            a DER value holding at least one CRL
	 * @exception CRLException
	 *                on parsing errors.
	 */
	 ...
	 * 
	 * @param inStrm
	 *            an input stream holding at least one CRL
	 * @exception CRLException
	 *                on parsing errors.
	 */
	 ...
	 * 
	 * @param issuer
	 *            the name of the CA issuing this CRL.
	 * @param thisUpdate
	 ...
	 *            the Date of the next CRL.
	 * @param badCerts
	 *            the array of CRL entries.
	 * 
	 * @exception CRLException
	 *                on parsing/construction errors.
	 */
	public X509CRLImpl(X500Name issuer, Date thisDate, Date nextDate,
			X509CRLEntry[] badCerts) throws CRLException {
		this.issuer = issuer;
		this.thisUpdate = thisDate;
		this.nextUpdate = nextDate;
		if (badCerts != null) {
	 ...
	 */
	public X509CRLImpl(X500Name issuer, Date thisDate, Date nextDate,
			X509CRLEntry[] badCerts, CRLExtensions crlExts) throws CRLException {
	...
	/**
	 * Verifies that this CRL was signed using the private key that corresponds
	 * to the given public key, and that the signature verification was computed
	...
	public String toString() {
		...
		if (revokedCerts.isEmpty())
			...
			int i = 1;
			for (Iterator<X509CRLEntry> iter = revokedCerts.values().iterator(); iter
					.hasNext(); i++)
	...
	/**
	 * Return the issuer as X500Principal. Overrides method in X509CRL to
	 * provide a slightly more efficient version.
	 */
	 ...
	 * signature algorithm defined in <a
	 * href="http://www.ietf.org/rfc/rfc3279.txt">RFC 3279: Algorithms and
	 ...
	 * 
	 * @returns AuthorityKeyIdentifier or null (if no
	 ...
	 * 
	 * @returns IssuingDistributionPointExtension or null (if no such extension)
	 * @throws IOException
	 *             on error
	 */
	...
	/**
	 * Gets a Set of the extension(s) marked CRITICAL in the CRL. In the
	...
	/**
	 * Gets a Set of the extension(s) marked NON-CRITICAL in the CRL. In the
	 * returned set, each extension is represented by its OID string.
	 * 
	 ...
	 * 
	 * @param oid
	 *            ObjectIdentifier of extension desired
	...
	/**
	 * Extract the issuer X500Principal from an X509CRL. Parses the encoded form
	 * of the CRL to preserve the principal's ASN.1 encoding.
	 * 
	...
	/**
	 * Utility method to convert an arbitrary instance of X509CRL to a
	 * X509CRLImpl. Does a cast if possible, otherwise reparses the encoding.
	 */
	...
	/**
	 * Return an interned X509CRLImpl for the given certificate. For more
	 * information, see intern(X509Certificate).
	 */
	 ...
	 * @param entry
	 *            the entry to check
	 * @param prevCertIssuer
	 *            the previous entry's certificate issuer
	...
	private X500Principal getCertIssuer(X509CRLEntryImpl entry,
		...
		CertificateIssuerExtension ciExt = entry
				.getCertificateIssuerExtension();
		if (ciExt != null) {
			GeneralNames names = (GeneralNames) ciExt
					.get(CertificateIssuerExtension.ISSUER);
	...
	private final static class X509IssuerSerial {
		...
		/**
		 * Compares this X509Serial with another and returns true if they are
		 * equivalent.
		 * 
		 * @param o
