...
public class X509CertInfo implements CertAttrSet<String> {
	 ...
	 * 
	 * @param cert
	 *            the encoded bytes, with no trailing data.
	...
	public X509CertInfo(byte[] cert) throws CertificateParsingException {
		try {
			...
			CertificateParsingException parseException = new CertificateParsingException(
					e.toString());
	 ...
	 * 
	 * @param out
	 *            an output stream to which the certificate is appended.
	...
	public String toString() {
		...
		if (extensions != null) {
			...
			Map<String, Extension> invalid = extensions
	...
	private void parse(DerValue val) throws CertificateParsingException,
		...
		X500Name issuerDN = (X500Name) issuer
		...
		X500Name subjectDN = (X500Name) subject
