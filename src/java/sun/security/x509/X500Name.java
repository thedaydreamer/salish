...
/**
 * Note: As of 1.4, the public class, javax.security.auth.x500.X500Principal,
 * should be used when parsing, generating, and comparing X.500 DNs. This class
 * contains other useful methods for checking name constraints and retrieving
 * DNs by keyword.
 * 
 * <p>
 ...
 * <ol TYPE=a>
 * <li>attribute values encoded in different types (e.g., PrintableString and
 * BMPString) may be assumed to represent different strings;
 * <p>
 * <li>attribute values in types other than PrintableString are case sensitive
 * (this permits matching of attribute values as binary objects);
 * <p>
 * <li>attribute values in PrintableString are not case sensitive (e.g.,
 * "Marianne Swanson" is the same as "MARIANNE SWANSON"); and
 * <p>
 * <li>attribute values in PrintableString are compared after removing leading
 * and trailing white space and converting internal substrings of one or more
 * consecutive white space characters to a single space.
 * </ol>
 * <p>
 ...
 * certificate user.
 * <p>
 * In addition, implementations of this specification MAY use these comparison
 * rules to process unfamiliar attribute types for name chaining. This allows
 * implementations to process certificates with unfamiliar attributes in the
 * issuer name.
 * <p>
 * Note that the comparison rules defined in the X.500 series of specifications
 * indicate that the character sets used to encode data in distinguished names
 * are irrelevant. The characters themselves are compared without regard to
 * encoding. Implementations of the profile are permitted to use the comparison
 * algorithm defined in the X.500 series. Such an implementation will recognize
 * a superset of name matches recognized by the algorithm specified above.
 * <p>
...
public class X500Name implements GeneralNameInterface, Principal {
	 ...
	 * "CN=Dave, OU=JavaSoft, O=Sun Microsystems, C=US". (RFC 1779 or RFC 2253
	 * style).
	 * 
	 * @param DN
	...
	/**
	 * Constructs a name from a string formatted according to format. Currently,
	 * the formats DEFAULT and RFC2253 are supported. DEFAULT is the default
	 * format used by the X500Name(String) constructor. RFC2253 is format
	 ...
	 * 
	 * <P>
	 * <EM><STRONG>NOTE:</STRONG>  The behaviour when any of
	...
	public X500Name(String commonName, String organizationUnit,
		...
		names[2] = new RDN(1);
		names[2].assertion[0] = new AVA(orgUnitName_oid, new DerValue(
				organizationUnit));
		names[1] = new RDN(1);
		names[1].assertion[0] = new AVA(orgName_oid, new DerValue(
				organizationName));
		names[0] = new RDN(1);
	...
	public X500Name(String commonName, String organizationUnit,
		...
		names[5].assertion[0] = new AVA(commonName_oid,
				new DerValue(commonName));
		names[4] = new RDN(1);
		names[4].assertion[0] = new AVA(orgUnitName_oid, new DerValue(
				organizationUnit));
		names[3] = new RDN(1);
		names[3].assertion[0] = new AVA(orgName_oid, new DerValue(
				organizationName));
		names[2] = new RDN(1);
		names[2].assertion[0] = new AVA(localityName_oid, new DerValue(
				localityName));
		names[1] = new RDN(1);
	...
	/**
	 * Return an immutable List of the the AVAs contained in all the RDNs of
	 * this X500Name.
	 */
	...
	/**
	 * Return the total number of AVAs contained in all the RDNs of this
	 * X500Name.
	 */
	public int avaSize() {
		return allAvas().size();
	...
	/**
	 * Return whether this X500Name is empty. An X500Name is not empty if it has
	 * at least one RDN containing at least one AVA.
	 */
	...
	/**
	 * Returns a "Domain" name component. If more than one such component
	 * exists, the topmost one is returned.
	 * 
	...
	/**
	 * Returns a "DN Qualifier" name component. If more than one such component
	 * exists, the topmost one is returned.
	 * 
	...
	/**
	 * Returns a "Surname" name component. If more than one such component
	 * exists, the topmost one is returned.
	 * 
	...
	/**
	 * Returns a "Given Name" name component. If more than one such component
	 * exists, the topmost one is returned.
	 * 
	...
	/**
	 * Returns an "Initials" name component. If more than one such component
	 * exists, the topmost one is returned.
	 * 
	...
	/**
	 * Returns a "Generation Qualifier" name component. If more than one such
	 * component exists, the topmost one is returned.
	 * 
	...
	/**
	 * Returns an "IP address" name component. If more than one such component
	 * exists, the topmost one is returned.
	 * 
	...
	/**
	 * Returns a string form of the X.500 distinguished name. The format of the
	 * string is from RFC 1779. The returned string may contain non-standardised
	 * keywords for more readability (keywords from RFCs 1779, 2253, and 3280).
	 */
	...
	/**
	 * Returns a string form of the X.500 distinguished name using the algorithm
	 * defined in RFC 1779. Only standard attribute type keywords defined in RFC
	 * 1779 are emitted.
	 */
	...
	/**
	 * Returns a string form of the X.500 distinguished name using the algorithm
	 * defined in RFC 1779. Attribute type keywords defined in RFC 1779 are
	 * emitted, as well as additional keywords contained in the OID/keyword map.
	 */
	...
	/**
	 * Returns a string form of the X.500 distinguished name using the algorithm
	 * defined in RFC 2253. Only standard attribute type keywords defined in RFC
	 * 2253 are emitted.
	 */
	...
	/**
	 * Returns a string form of the X.500 distinguished name using the algorithm
	 * defined in RFC 2253. Attribute type keywords defined in RFC 2253 are
	 * emitted, as well as additional keywords contained in the OID/keyword map.
	 */
	...
	private String generateRFC2253DN(Map<String, String> oidMap) {
		/*
		 * Section 2.1 : if the RDNSequence is an empty sequence the result is
		 * the empty or zero length string.
		 */
		if (names.length == 0) {
			return "";
		}
	...
	/**
	 * Returned the encoding as an uncloned byte array. Callers must guarantee
	 * that they neither modify it not expose it to untrusted code.
	 */
	 ...
	 * 
	 * According to RFC 1779, RDNs in a DN are separated by comma. The following
	 * examples show both methods of quoting a comma, so that it is not
	 * considered a separator:
	 * 
	 * O="Sue, Grabbit and Runn" or O=Sue\, Grabbit and Runn
	 * 
	...
	private void parseDN(String input, Map<String, String> keywordMap)
		...
		while (nextComma >= 0 || nextSemiColon >= 0) {
			...
			/*
			 * We have encountered an RDN delimiter (comma or a semicolon). If
			 * the comma or semicolon in the RDN under consideration is preceded
			 * by a backslash (escape), or by a double quote, it is part of the
			 * RDN. Otherwise, it is used as a separator, to delimit the RDN
			 * under consideration from any subsequent RDNs.
			 */
		...
		/*
		 * Store the vector elements as an array of RDNs NOTE: It's only on
		 * output that little-endian ordering is used.
		 */
	...
	private void parseRFC2253DN(String dnString) throws IOException {
		...
		while (rdnEnd >= 0) {
			/*
			 * We have encountered an RDN delimiter (comma). If the comma in the
			 * RDN under consideration is preceded by a backslash (escape), it
	 ...
	 * have "label=value" syntax.
	 * 
	 * Uses RFC 1779 syntax (i.e. little-endian, comma separators) Valid
	 * keywords from RFC 1779 are used. Additional keywords can be specified in
	 * the OID/keyword map.
	 */
	...
	/*
	 * Selected OIDs from X.520 Includes all those specified in RFC 3280 as MUST
	...
	private static final int ipAddress_data[] = { 1, 3, 6, 1, 4, 1, 42, 2, 11,
			2, 1 };
	private static final int DOMAIN_COMPONENT_DATA[] = { 0, 9, 2342, 19200300,
			100, 1, 25 };
	...
	static {
		 ...
		 */
		SERIALNUMBER_OID = intern(ObjectIdentifier
				.newInternal(SERIALNUMBER_DATA));
		...
		localityName_oid = intern(ObjectIdentifier
				.newInternal(localityName_data));
		...
		streetAddress_oid = intern(ObjectIdentifier
				.newInternal(streetAddress_data));
		...
		GENERATIONQUALIFIER_OID = intern(ObjectIdentifier
				.newInternal(GENERATIONQUALIFIER_DATA));

		/*
		 * OIDs from other sources which show up in X.500 names we expect to
		 * deal with often
		 */
		 ...
		 * format
		 */
		DOMAIN_COMPONENT_OID = intern(ObjectIdentifier
				.newInternal(DOMAIN_COMPONENT_DATA));
	 ...
	 * but is same type
	 * </ul>
	 * . These results are used in checking NameConstraints during certification
	 * path verification.
	 * 
	 * @param inputName
	...
	/**
	 * Compares this name with another and determines if it is within the
	 * subtree of the other. Useful for checking against the name constraints
	 * extension.
	 * 
	 ...
	 * NameConstraints minimum and maximum bounds and for calculating path
	 * lengths in name subtrees.
	 * 
