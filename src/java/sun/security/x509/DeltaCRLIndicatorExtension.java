 ...
 * 
 * <p>
 * The extension identifies a CRL as being a delta CRL. Delta CRLs contain
 * updates to revocation information previously distributed, rather than all the
 * information that would appear in a complete CRL. The extension contains a CRL
 * number that identifies the CRL, complete for a given scope, that was used as
 * the starting point in the generation of this delta CRL.
 * 
 * <p>
 * The extension is defined in Section 5.2.4 of <a
...
public class DeltaCRLIndicatorExtension extends CRLNumberExtension {
	...
	/**
	 * Creates a delta CRL indicator extension with the integer value . The
	 * criticality is set to true.
	 * 
	 * @param crlNum
	 *            the value to be set for the extension.
	 */
	public DeltaCRLIndicatorExtension(int crlNum) throws IOException {
		super(PKIXExtensions.DeltaCRLIndicator_Id, true, BigInteger
				.valueOf(crlNum), NAME, LABEL);
	}

	/**
	 * Creates a delta CRL indictor extension with the BigInteger value . The
	 * criticality is set to true.
	 * 
	 * @param crlNum
	 *            the value to be set for the extension.
