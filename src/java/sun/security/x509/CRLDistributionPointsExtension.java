 ...
 * obtained. The extension SHOULD be non-critical, but the PKIX profile
 ...
 * the URI is a pointer to the current CRL for the associated reasons and will
 * be issued by the associated cRLIssuer. The expected values for the URI
 * conform to the following rules. The name MUST be a non-relative URL, and MUST
 * follow the URL syntax and encoding rules specified in [RFC 1738]. The name
 * must include both a scheme (e.g., "http" or "ftp") and a
 * scheme-specific-part. The scheme- specific-part must include a fully
 * qualified domain name or IP address as the host. As specified in [RFC 1738],
 * the scheme name is not case-sensitive (e.g., "http" is equivalent to "HTTP").
 * The host part is also not case-sensitive, but other components of the
 * scheme-specific-part may be case-sensitive. When comparing URIs, conforming
 * implementations MUST compare the scheme and host without regard to case, but
 * assume the remainder of the scheme-specific-part is case sensitive.
 * Processing rules for other values are not defined by this specification. If
 * the distributionPoint omits reasons, the CRL MUST include revocations for all
 * reasons. If the distributionPoint omits cRLIssuer, the CRL MUST be issued by
...
public class CRLDistributionPointsExtension extends Extension implements
	 ...
	 *            the list of distribution points
	 * @throws IOException
	 *             on error
	 */
	...
	public void set(String name, Object obj) throws IOException {
		if (name.equalsIgnoreCase(POINTS)) {
			...
			throw new IOException("Attribute name [" + name
					+ "] not recognized by " + "CertAttrSet:" + extensionName
					+ ".");
		}
		encodeThis();
	}
