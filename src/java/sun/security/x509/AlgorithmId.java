...
public class AlgorithmId implements Serializable, DerEncoder {
	...
	public void derEncode(OutputStream out) throws IOException {
		...
		if (params == null) {
			...
			/*
			 * if ( algid.equals((Object)SHA_oid) ||
			 * algid.equals((Object)SHA256_oid) ||
			 * algid.equals((Object)SHA384_oid) ||
			 * algid.equals((Object)SHA512_oid) || algid.equals((Object)DSA_oid)
			 * || algid.equals((Object)sha1WithDSA_oid)) { ; // no parameter
			 * part encoded } else { bytes.putNull(); }
			 */
			bytes.putNull();
		} else {
	...
	public String getName() {
		...
		if ((params != null) && algid.equals(specifiedWithECDSA_oid)) {
			try {
				AlgorithmId paramsId = AlgorithmId.parse(new DerValue(
						getEncodedParams()));
	...
	public boolean equals(AlgorithmId other) {
		boolean paramsEqual = (params == null ? other.params == null : params
				.equals(other.params));
	 ...
	 *                on error.
	 * @param val
	 *            the input value, which contains the algid and, if there are
	...
	public static AlgorithmId get(String algname)
		...
		try {
			...
			throw new NoSuchAlgorithmException("Invalid ObjectIdentifier "
					+ algname);
		}
	 ...
	 * 
	 * XXX This method needs to be enhanced so that we can also pass the scope
	 * of the algorithm name to it, e.g., the algorithm name "DSA" may have a
	 * different OID when used as a "Signature" algorithm than when used as a
	...
	private static ObjectIdentifier algOID(String name) throws IOException {
		...
		if (!initOidTable) {
			...
			for (int i = 0; i < provs.length; i++) {
				for (Enumeration<Object> enum_ = provs[i].keys(); enum_
						.hasMoreElements();) {
					...
					if (alias.toUpperCase().startsWith("ALG.ALIAS")
						...
						if (oidTable.get(stdAlgName) == null) {
							oidTable.put(stdAlgName, new ObjectIdentifier(
									oidString));
						}
					}
				}
	 ...
	 * 1.2.840.113549.2.2
	 */
	public static final ObjectIdentifier MD2_oid = ObjectIdentifier
			.newInternal(new int[] { 1, 2, 840, 113549, 2, 2 });

	/**
	 * Algorithm ID for the MD5 Message Digest Algorthm, from RFC 1321. OID =
	 * 1.2.840.113549.2.5
	 */
	public static final ObjectIdentifier MD5_oid = ObjectIdentifier
			.newInternal(new int[] { 1, 2, 840, 113549, 2, 5 });
	 ...
	 * 1.3.14.3.2.26. Old SHA-0 OID: 1.3.14.3.2.18.
	 */
	public static final ObjectIdentifier SHA_oid = ObjectIdentifier
			.newInternal(new int[] { 1, 3, 14, 3, 2, 26 });
	...
	private static final int sha256WithRSAEncryption_data[] = { 1, 2, 840,
			113549, 1, 1, 11 };
	private static final int sha384WithRSAEncryption_data[] = { 1, 2, 840,
			113549, 1, 1, 12 };
	...
	public static final ObjectIdentifier sha1WithECDSA_oid = oid(1, 2, 840,
			10045, 4, 1);
	public static final ObjectIdentifier sha224WithECDSA_oid = oid(1, 2, 840,
			10045, 4, 3, 1);
	public static final ObjectIdentifier sha256WithECDSA_oid = oid(1, 2, 840,
			10045, 4, 3, 2);
	public static final ObjectIdentifier sha384WithECDSA_oid = oid(1, 2, 840,
			10045, 4, 3, 3);
	public static final ObjectIdentifier sha512WithECDSA_oid = oid(1, 2, 840,
			10045, 4, 3, 4);
	public static final ObjectIdentifier specifiedWithECDSA_oid = oid(1, 2,
			840, 10045, 4, 3);
	...
	static {
		/*
		 * Note the preferred OIDs are named simply with no "OIW" or "PKIX" in
		 * them, even though they may point to data from these specs; e.g.
		 * SHA_oid, DH_oid, DSA_oid, SHA1WithDSA_oid...
		 */
		/**
		 ...
		 * certificate. OID = 1.2.840.113549.1.3.1
		 */
		DH_oid = ObjectIdentifier.newInternal(DH_data);

		/**
		 * Algorithm ID for the Diffie Hellman Key Agreement (DH), from RFC
		 * 3279. Parameters may include public values P and G. OID =
		 * 1.2.840.10046.2.1
		 */
		DH_PKIX_oid = ObjectIdentifier.newInternal(DH_PKIX_data);

		/**
		 * Algorithm ID for the Digital Signing Algorithm (DSA), from the NIST
		 * OIW Stable Agreements part 12. Parameters may include public values
		 * P, Q, and G; or these may be derived from another source such as a
		 * Certificate Authority's certificate. OID = 1.3.14.3.2.12
		 */
		 ...
		 * public modulus. OID = 1.2.5.8.1.1
		 */
		RSA_oid = ObjectIdentifier.newInternal(RSA_data);

		/**
		 * Algorithm ID for RSA keys used with RSA encryption, as defined in
		 * PKCS #1. There are no parameters associated with this algorithm. OID
		 * = 1.2.840.113549.1.1.1
		 */
		RSAEncryption_oid = ObjectIdentifier.newInternal(RSAEncryption_data);

		/**
		 * Identifies a signing algorithm where an MD2 digest is encrypted using
		 * an RSA private key; defined in PKCS #1. Use of this signing algorithm
		 * is discouraged due to MD2 vulnerabilities. OID = 1.2.840.113549.1.1.2
		 */
		md2WithRSAEncryption_oid = ObjectIdentifier
				.newInternal(md2WithRSAEncryption_data);

		/**
		 * Identifies a signing algorithm where an MD5 digest is encrypted using
		 * an RSA private key; defined in PKCS #1. OID = 1.2.840.113549.1.1.4
		 */
		md5WithRSAEncryption_oid = ObjectIdentifier
				.newInternal(md5WithRSAEncryption_data);

		/**
		 * Identifies a signing algorithm where a SHA1 digest is encrypted using
		 * an RSA private key; defined by RSA DSI. OID = 1.2.840.113549.1.1.5
		 */
		sha1WithRSAEncryption_oid = ObjectIdentifier
				.newInternal(sha1WithRSAEncryption_data);

		/**
		 * Identifies a signing algorithm where a SHA1 digest is encrypted using
		 * an RSA private key; defined in NIST OIW. OID = 1.3.14.3.2.29
		 */
		sha1WithRSAEncryption_OIW_oid = ObjectIdentifier
				.newInternal(sha1WithRSAEncryption_OIW_data);

		/**
		 * Identifies a signing algorithm where a SHA256 digest is encrypted
		 * using an RSA private key; defined by PKCS #1. OID =
		 * 1.2.840.113549.1.1.11
		 */
		sha256WithRSAEncryption_oid = ObjectIdentifier
				.newInternal(sha256WithRSAEncryption_data);

		/**
		 * Identifies a signing algorithm where a SHA384 digest is encrypted
		 * using an RSA private key; defined by PKCS #1. OID =
		 * 1.2.840.113549.1.1.12
		 */
		sha384WithRSAEncryption_oid = ObjectIdentifier
				.newInternal(sha384WithRSAEncryption_data);

		/**
		 * Identifies a signing algorithm where a SHA512 digest is encrypted
		 * using an RSA private key; defined by PKCS #1. OID =
		 * 1.2.840.113549.1.1.13
		 */
		sha512WithRSAEncryption_oid = ObjectIdentifier
				.newInternal(sha512WithRSAEncryption_data);

		/**
		 * Identifies the FIPS 186 "Digital Signature Standard" (DSS), where a
		 * SHA digest is signed using the Digital Signing Algorithm (DSA). This
		 * should not be used. OID = 1.3.14.3.2.13
		 */
		shaWithDSA_OIW_oid = ObjectIdentifier.newInternal(shaWithDSA_OIW_data);

		/**
		 * Identifies the FIPS 186 "Digital Signature Standard" (DSS), where a
		 * SHA1 digest is signed using the Digital Signing Algorithm (DSA). OID
		 * = 1.3.14.3.2.27
		 */
		sha1WithDSA_OIW_oid = ObjectIdentifier
				.newInternal(sha1WithDSA_OIW_data);
