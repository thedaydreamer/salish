 ...
 * label, the domain name MUST be stored in the dNSName (an IA5String). The name
 ...
 * signifigance is attached to the case. In addition, while the string " " is a
 * legal domain name, subjectAltName extensions with a dNSName " " are not
 * permitted. Finally, the use of the DNS representation for Internet mail
 * addresses (wpolk.nist.gov instead of wpolk@nist.gov) is not permitted; such
 * identities are to be encoded as rfc822Name.
 * <p>
 * 
 * @author Amit Kapoor
...
public class DNSName implements GeneralNameInterface {
	 ...
	 *            the DNSName.
	 * @throws IOException
	 *             if the name is not a valid DNSName subjectAltName
	 */
	public DNSName(String name) throws IOException {
		...
		for (int endIndex, startIndex = 0; startIndex < name.length(); startIndex = endIndex + 1) {
			...
			// nonStartIndex: index for characters in the component beyond the
			// first one
	 ...
	 * 
	 * @param out
	 *            the DER stream to encode the DNSName to.
	 * @exception IOException
	 *                on encoding errors.
	 */
	 ...
	 * path verification.
	 * <p>
	 * RFC2459: DNS name restrictions are expressed as foo.bar.com. Any
	 * subdomain satisfies the name constraint. For example, www.foo.bar.com
	 * would satisfy the constraint but bigfoo.bar.com would not.
	 * <p>
	 * draft-ietf-pkix-new-part1-00.txt: DNS name restrictions are expressed as
	 * foo.bar.com. Any DNS name that can be constructed by simply adding to the
	 * left hand side of the name satisfies the name constraint. For example,
	 * www.foo.bar.com would satisfy the constraint but foo1.bar.com would not.
	 * <p>
	 * RFC1034: By convention, domain names can be stored with arbitrary case,
	 * but domain name comparisons for all present domain functions are done in
	 * a case-insensitive manner, assuming an ASCII character set, and a high
	 * order zero bit.
	 * <p>
	 * 
	 * @param inputName
