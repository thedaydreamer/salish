...
final public class ObjectIdentifier implements Serializable {
	...
	/**
	 * Check if the values make a legal OID. There must be at least 2 components
	 * and they must be all non-negative. The first component should be 0,1 or
	 * 2. When the first component is 0 or 1, the second component should be
	 * less than or equal to 39
	 * 
	 * @param values
	 *            the components that will make the OID
	 * @param len
	 *            the number of components to check. Note that the allocation
	 *            size of <code>values</code> may be longer than
	 *            <code>len</code>. In this case, only the first
	 *            <code>len</code> items are checked.
	 * @exception IOException
	 *                if this is not a legal OID
	 */
	...
	/**
	 * Private constructor for use by newInternal(). Dummy argument to avoid
	 * clash with the public constructor.
	 */
	...
	/**
	 * Create a new ObjectIdentifier for internal use. The values are neither
	 * checked nor cloned.
	 */
