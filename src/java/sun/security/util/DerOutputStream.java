...
public class DerOutputStream extends ByteArrayOutputStream implements
	...
	private void putIntegerContents(int i) throws IOException {
		...
		if (bytes[0] == 0xff) {
			...
			for (int j = 0; j < 3; j++) {
				if ((bytes[j] == 0xff) && ((bytes[j + 1] & 0x80) == 0x80))
					start++;
				else
					break;
			}
	...
	/**
	 * Marshals a truncated DER bit string on the output stream. The bit strings
	 ...
	 *            the string to write
	 * @param stringTag
	 *            one of the DER string tags that indicate which encoding should
	 *            be used to write the string out.
	 * @param enc
	 *            the name of the encoder that should be used corresponding to
	 *            the above tag.
	 */
	 ...
	 * seconds (even if seconds=0) as per RFC 3280.
	 */
	...
	/**
	 * Private helper routine for marshalling a DER UTC/Generalized time/date
	 * value. If the tag specified is not that for UTC Time then it defaults to
	 * Generalized Time.
	 * 
	 * @param d
