...
class DerInputBuffer extends ByteArrayInputStream implements Cloneable {
	 ...
	 * @param makePositive
	 *            whether to always return a positive value, irrespective of
	 *            actual encoding
	 ...
	 * 
	 * @throws IOException
	 *             if the result is not within the valid range for integer, i.e.
	...
	BitArray getUnalignedBitString() throws IOException {
		...
		if (unusedBits > 7) {
			throw new IOException("Invalid value for unused bits: "
					+ unusedBits);
		}
	...
	/**
	 * Returns the UTC Time value that takes up the specified number of bytes in
	 * this buffer.
	 * 
	 * @param len
	 *            the number of bytes to use
	...
	/**
	 * Returns the Generalized Time value that takes up the specified number of
	 ...
	 *            the number of bytes to use
	 * @param generalized
	 *            true if Generalized Time is to be read, false if UTC Time is
	 *            to be read.
	 */
	private Date getTime(int len, boolean generalized) throws IOException {

		/*
		 * UTC time encoded as ASCII chars: YYMMDDhhmmZ YYMMDDhhmmssZ
		 * YYMMDDhhmm+hhmm YYMMDDhhmm-hhmm YYMMDDhhmmss+hhmm YYMMDDhhmmss-hhmm
		 * UTC Time is broken in storing only two digits of year. If YY < 50, we
		 * assume 20YY; if YY >= 50, we assume 19YY, as per RFC 3280.
		 * 
		 * Generalized time has a four-digit year and allows any precision
		 * specified in ISO 8601. However, for our purposes, we will only allow
		 * the same format as UTC time, except that fractional seconds
		 * (millisecond precision) are supported.
		 */
		...
		if (month == 0 || day == 0 || month > 12 || day > 31 || hour >= 24
				|| minute >= 60 || second >= 60)
			throw new IOException("Parse " + type + " time, invalid format");

		/*
		 * Generalized time can theoretically allow any precision, but we're not
		 * supporting that.
		 */
		/*
		 * out:kellinwood CalendarSystem gcal =
		 * CalendarSystem.getGregorianCalendar(); CalendarDate date =
		 * gcal.newCalendarDate(null); // no time zone date.setDate(year, month,
		 * day); date.setTimeOfDay(hour, minute, second, millis); long time =
		 * gcal.getTime(date);
		 */
