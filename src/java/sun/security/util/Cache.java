 ...
 * 
 * The factory methods can be used to obtain two different implementations. They
 ...
 * SoftReferences have the advantage that they are automatically cleared by the
 * garbage collector in response to memory demand. This makes it possible to
 * simple set the maximum size to a very large value and let the GC
 * automatically size the cache dynamically depending on the amount of available
 * memory.
 * 
 ...
 * 
 * Cache sizing: the memory cache is implemented on top of a LinkedHashMap. In
 * its current implementation, the number of buckets (NOT entries) in
 * (Linked)HashMaps is always a power of two. It is recommended to set the
 * maximum cache size to value that uses those buckets fully. For example, if a
 * cache with somewhere between 500 and 1000 entries is desired, a maximum size
 * of 750 would be a good choice: try 1024 buckets, with a load factor of 0.75f,
 * the number of entries can be calculated as buckets / 4 * 3. As mentioned
 * above, with a SoftReference cache, it is generally reasonable to set the size
 * to a fairly large value.
 * 
...
public abstract class Cache {
	...
	/**
	 * Return a new memory cache with the specified maximum size, the specified
	 * maximum lifetime (in seconds), with the values held by SoftReferences.
	 */
	...
	/**
	 * Return a new memory cache with the specified maximum size, the specified
	 * maximum lifetime (in seconds), with the values held by standard
	 * references.
	 */
	...
	/**
	 * Utility class that wraps a byte array and implements the equals() and
	 * hashCode() contract in a way suitable for Maps and caches.
	 */
...
class MemoryCache extends Cache {
	...
	private static class SoftCacheEntry extends SoftReference implements
			CacheEntry {
