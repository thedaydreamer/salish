...
public class DerInputStream {
	 ...
	 * IA5String, UTCTime, GeneralizedTime, BMPString. Note: UniversalString not
	 * supported till encoder is available.
	 */
	 ...
	 * 
	 * @param stringTag
	 *            the tag for the type of string to read
	 * @param stringName
	 *            a name to display in error messages
	 * @param enc
	 *            the encoder to use to interpret the data. Should correspond to
	 *            the stringTag above.
	 */
	private String readString(byte stringTag, String stringName, String enc)
			throws IOException {
	...
	static int getLength(int lenByte, InputStream in) throws IOException {
		...
		if ((tmp & 0x080) == 0x00) { // short form, 1 byte datum
			...
			/*
			 * NOTE: tmp == 0 indicates indefinite length encoded data. tmp > 4
			 * indicates more than 4Gb of data.
			 */
			if (tmp == 0)
				return -1;
			if (tmp < 0 || tmp > 4)
