...
public class ManifestDigester {
	 ...
	 * 
	 * @param offset
	 *            should point to the starting offset with in the raw bytes of
	 *            the next section.
	 * 
	 * @pos set by
	 * 
	 * @returns false if end of bytes has been reached, otherwise returns true
	 */
	...
	public ManifestDigester(byte bytes[]) {
		...
		entries.put(MF_MAIN_ATTRS, new Entry(0, pos.endOfSection + 1,
				pos.startOfNext, rawBytes));
			...
			if (len > 6) {
				if (isNameAttr(bytes, start)) {
					...
					try {
						...
						entries.put(nameBuf.toString(), new Entry(start,
								sectionLen, sectionLenWithBlank, rawBytes));
	...
	public static class Entry {
		...
		public Entry(int offset, int length, int lengthWithBlankLine,
				byte[] rawBytes) {
			this.offset = offset;
			this.length = length;
		...
		private void doOldStyle(MessageDigest md, byte[] bytes, int offset,
				int length) {
