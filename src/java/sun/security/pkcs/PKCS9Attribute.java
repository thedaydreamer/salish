...
/**
 * Class supporting any PKCS9 attributes. Supports DER decoding and access to
 * attribute values, but not DER encoding or setting of values.
 * 
 * <a name="classTable"><h3>Type/Class Table</h3></a> The following table shows
 * the correspondence between PKCS9 attribute types and value component classes.
 * 
 * <P>
...
public class PKCS9Attribute implements DerEncoder {
	...
	static { // static initializer for PKCS9_OIDS
		for (int i = 1; i < PKCS9_OIDS.length - 2; i++) {
			PKCS9_OIDS[i] = ObjectIdentifier.newInternal(new int[] { 1, 2, 840,
					113549, 1, 9, i });
		}
	 ...
	 * multi-valued, provide an array containing all the values. Arrays of
	 * length zero are accepted, though probably useless.
	 * 
	 * <P>
	 * The <a href=#classTable>table</a> gives the class that <code>value</code>
	 ...
	 * <P>
	 * The <a href=#classTable>table</a> gives the class that <code>value</code>
	 * must have for a given attribute. Reasonable variants of these attributes
	 * are accepted; in particular, case does not matter.
	 * 
	...
	public PKCS9Attribute(DerValue derVal) throws IOException {
		...
		case 14: // ExtensionRequest
			value = new CertificateExtensions(new DerInputStream(
					elems[0].toByteArray()));
			break;
	 ...
	 * return just the one value. If the attribute is multi-valued, return an
	 * array containing all the values. It is possible for this array to be of
	 * length 0.
	 * 
	 * <P>
	 * The <a href=#classTable>table</a> gives the class of the value returned,
