...
public abstract class CharacterEncoder {
	 ...
	 * the output stream. This method will run until it exhausts the input
	 * stream, but does not print the line suffix for a final line that is
	 ...
	 * byte array backing the ByteBuffer. If this is not possible, a new byte
	 * array will be created.
	 */
	private byte[] getBytes(ByteBuffer bb) {
		/*
		 * This should never return a BufferOverflowException, as we're careful
		 * to allocate just the right amount.
		 */
		byte[] buf = null;
		...
		if (buf == null) {
			/*
			 * This class doesn't have a concept of encode(buf, len, off), so if
			 * we have a partial buffer, we must reallocate space.
			 */
	...
	/**
	 * A 'streamless' version of encode that simply takes a ByteBuffer and
	 * returns a string containing the encoded buffer.
	 * <P>
	 * The ByteBuffer's position will be advanced to ByteBuffer's limit.
	 */
	 ...
	 * Encode bytes from the input stream, and write them as text characters to
	 * the output stream. This method will run until it exhausts the input
	 * stream. It differs from encode in that it will add the line at the end of
